...
parameters {
  ...
  real<lower=lowBound[8],upper=upBound[8]> vmax[P];
  real<lower=lowBound[3],upper=upBound[3]> bG;                // volume productivity increase rate
  real<lower=lowBound[4],upper=bG*(max(vmax)*theta/aM+1)> bM;                  // volume mortality increase rate
  ...
  // hyperparameters of the parameter vmax distribution
  real<lower=lowBound[9], upper=upBound[9]> mu_vmax;
  real<lower=0, upper=supSigma[5]> sigma_vmax;
}
...
model{
  ...
  // hyperdistributions
 vmax ~ normal(mu_vmax, sigma_vmax);
